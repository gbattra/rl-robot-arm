...
args = gymutil.parse_arguments(
    description="Pick Box Experiment"
)
...
for i in range(num_envs):
    ...
    gym.set_rigid_body_color(env, box_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, blue)

    # add target
    t_box_pose.p.x = table_pose.p.x + np.random.uniform(-0.2, 0.1)
    t_box_pose.p.y = table_pose.p.y + np.random.uniform(-0.3, 0.3)
    t_box_pose.p.z = table_dims.z + 0.5 * box_size
    t_box_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 0, 1), np.random.uniform(-math.pi, math.pi))
    t_box_handle = gym.create_actor(env, box_asset, t_box_pose, "target", i, 0)
    red = gymapi.Vec3(1., .0, .0)
    gym.set_rigid_body_color(env, t_box_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, red)

    # get global index of box in rigid body state tensor
    box_idx = gym.get_actor_rigid_body_index(env, box_handle, 0, gymapi.DOMAIN_SIM)
    box_idxs.append(box_idx)

    # get global index of target in rigidbody state tensor
    target_idx = gym.get_actor_rigid_body_index(env, t_box_handle, 0, gymapi.DOMAIN_SIM)
...
dof_pos = dof_states[:, 0].view(num_envs, 9)
dof_vel = dof_states[:, 1].view(num_envs, 9)
...
# simulation loop
for p in trange(5, desc="Epoch", leave=False):
    for e in trange(100, desc="Episode", leave=False):
        for t in trange(500, desc="Step", leave=False):
            ###
            ### COMPUTE STATE
            ###
            ...
            target_pos = rb_states[target_idxs, :3]

            hand_pos = rb_states[hand_idxs, :3]
            hand_rot = rb_states[hand_idxs, 3:7]
            hand_vel = rb_states[hand_idxs, 7:]

            state = torch.cat((hand_pos, hand_rot, hand_vel, box_pos, target_pos), dim=-1)

            ###
            ### CHOOSE ACTIONS
            ###
            actions = (torch.rand((num_envs, 12), device=device).view(-1, 4, 3).max(-1)[1] - 1) * action_scale

            # convert actions to target eff position and orientation
            goal_pos = hand_pos + actions[:, :3]

            # compute position and orientation error
            pos_err = goal_pos - hand_pos
            orn_err = orientation_error(down_q, hand_rot)
            dpose = torch.cat([pos_err, orn_err], -1).unsqueeze(-1)

            pos_action[:, :7] = dof_pos[:, :7] + control_ik(dpose)
            pos_action[:, 6] = dof_pos[:, 6] + actions[:, 3]
            ...
            ### COMPUTE NEXT STATE
            ###
            next_state = torch.cat((hand_pos, hand_rot, hand_vel, box_pos, target_pos), dim=-1)
