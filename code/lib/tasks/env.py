...
class ApproachBoxEnv(Env):
    ...
    def compute_rewards(self) -> torch.Tensor:
        ...
        lf_distances: torch.Tensor = torch.norm(
            self.right_finger_poses[:, 0:3] - self.box_poses[:, 0:3], p=2, dim=-1
        ).to(self.device)
        h_targets = self.box_poses.clone().to(self.device)
        h_targets[:, 2] += .125
        h_distances: torch.Tensor = torch.norm(
            self.hand_poses[:, 0:3] - h_targets[:, 0:3], p=2, dim=-1
        ).to(self.device)

        rwds: torch.Tensor = torch.ones((self.n_envs, 1)).to(self.device) * -0.01
        lf_close: torch.Tensor = lf_distances.le(0.2).to(self.device)
        lf_closer = lf_distances.le(0.1).to(self.device)
        lf_closest = lf_distances.le(0.05).to(self.device)
        lf_on_target = lf_distances.le(0.01).to(self.device)

        rf_close: torch.Tensor = lf_distances.le(0.2).to(self.device)
        rf_closer = lf_distances.le(0.1).to(self.device)
        rf_closest = lf_distances.le(0.05).to(self.device)
        rf_on_target = lf_distances.le(0.01).to(self.device)

        h_close: torch.Tensor = h_distances.le(0.2).to(self.device)
        h_closer = h_distances.le(0.1).to(self.device)
